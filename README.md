...
## 命令
    ...
    npm run build		//打包xy0
    npm run build 02	//打包xy2
    npm run release 先打包xy1,打包完成后自动打包xy2
    
    //app相关：
    npm start 00 app  开启xy0 app环境
    npm run build 01 app 打包xy1的app包
    后续也可以集体打包，但还是要一个一个复制到hbuilder里面打包
...
所有hashHistory 使用this.props.history 
...
个人中心有不同的图标 参考 `user/uer_login.js`

    import("../../img/pankou/"+pankou+"/icon_touxiang.png").then((module)=>{
        this.setState({
            imgUrl:module
        })
    })
...
所有连接到外部的地址，都不能使用 location.href 和 window.open 参考 `user/getService`和`common/appCommon`

另外 如果app要调用手机自带浏览器 使用 plus.runtime.openURL("要打开的连接地址");

支付相关要保存图片的话，要导入下载图片js和mui.min.js 参考 `user/pay.js`
