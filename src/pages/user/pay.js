    ...
    render() {
        ...
        initData.map((item, i) => {
            if (item.cdg_id === 1) {//银行卡特殊 公司入款
                ...
                />)
            }else if(item.cdg_id === 3){ //扫码
                renderHtml.push(<ScanCodeForm
                    onlineService={() => {
                        this.onlineService()
                    }}
                    data={item}
...
class ScanCodeForm extends Component {
    constructor(props) {
        super(props);
        this.state = {
            money: "",
    ...
    handleSubmit() {
        ...
        let token = getRandChar();
        let params = {}
        if(payInfo.pay_id_input===1){
            if(document.getElementById("payname")&&document.getElementById("payname").value !==""){
                params.pay_account_id = document.getElementById("payname").value
            }else{
                //实现不累加显示，重复点击只显示一个
                message.config({
                    top: 20,
        ...
        params.requestURI = payInfo.requestURI
        params.card_id = payInfo.card_id
        params.token = token
        params.money = money
        params.area_flag = 1
            MaskLoading(5);
            Api("c=pay&a=pay", params, (res) => {
                MaskLoading(false);
                if (res.errno === 0) {
                ...
                <div className="pay-content">
                    <div className="form-wrap">
                        ...
                        <div className="qrcode-img">
                            <img src={this.state.cardList[pipeIndex].netway} alt=""/>
                            <p>贴心提醒：收款帐号、收款姓名和二维码会不定期更换，请在获取页面最新信息后再进行充值，以避免充值无法到帐。
                                "充值金额"若与转帐金额不符，充值将无法准确到帐。</p>
                        </div>
                        {this.state.cardList[pipeIndex].pay_id_input===1?
                            <div className="form-item">
                                <label htmlFor="payname">支付账号 :</label>
                                <div>
                                    <input type="text" id="payname" placeholder="输入支付账号"/>
                                </div>
                            </div>
                            :null
                        }
